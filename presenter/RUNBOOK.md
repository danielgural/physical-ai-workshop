...
- [ ] demo.fiftyone.ai: `Droid demo` (100 episodes), `Droid workshop episodes` (the 10 attendees have, with brick-visible/gripper-visible tags) and `Droid workshop frames` (5,172 frames, all runs) load; Embeddings panel opens `frames_viz`; Model Evaluation opens `eval_yolo`.
...
| 25–40 | 9–10 | Show `build/extract_frames.py` for 60 s (H.264 → ffmpeg → JPEG, fields per frame). Switch to `Droid workshop frames`. Filter `gripper_open == False`, `camera == wrist`. Sort by `uniqueness` asc (empty bench) then desc (hands, dropped objects). `near-duplicate` tag: 356 frames. Build and save `curated_train`. | recording-curate.mp4 |
